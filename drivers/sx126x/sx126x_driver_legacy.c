...
static int sx126x_driver_send_async(const struct device *dev, uint8_t *data, uint32_t data_len, struct k_poll_signal *async) {
    const struct sx126x_hal_config_t *config = dev->config;
    int ret;
    ...
    if (config->rx_enable.port) {
        ret = gpio_pin_set_dt(&config->rx_enable, 0);
        if (ret < 0) {
			LOG_ERR("sx126x_driver_send_async():gpio_pin_set_dt(RX ENABLE) returned: %d", ret);
			return ret;
		}
    }

    return ral_driver_send_async(dev, data, data_len, async);
}
...
static int sx126x_driver_recv_async(const struct device *dev, lora_recv_cb cb, void *user_data) {
    const struct sx126x_hal_config_t *config = dev->config;
    int ret;
    ...
    if (config->rx_enable.port) {
        ret = gpio_pin_set_dt(&config->rx_enable, 1);
        if (ret < 0) {
			LOG_ERR("sx126x_driver_recv_async():gpio_pin_set_dt(RX ENABLE) returned: %d", ret);
			return ret;
		}
    }

    return ral_driver_recv_async(dev, cb, user_data);
}
...
static DEVICE_API(lora, sx126x_lora_api) = {
    .config     = ral_driver_config,
    .recv       = sx126x_driver_recv,
    .recv_async = sx126x_driver_recv_async,
    .send       = sx126x_driver_send,
    .send_async = sx126x_driver_send_async
};
