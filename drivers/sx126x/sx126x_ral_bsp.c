...
void ral_sx126x_bsp_get_tx_cfg(const void* context, const ral_sx126x_bsp_tx_cfg_input_params_t* input_params, ral_sx126x_bsp_tx_cfg_output_params_t* output_params) {
	// get board tx power offset
	int8_t board_tx_pwr_offset_db = DEFAULT_TX_POWER_OFFSET_DB;
	...
	power = CLAMP(power, SX126X_HP_MIN_OUTPUT_POWER, SX126X_HP_MAX_OUTPUT_POWER);
...
#else
	// Clamp power if needed
	power = CLAMP(power, SX126X_LP_MIN_OUTPUT_POWER, SX126X_LP_MAX_OUTPUT_POWER);
...
void ral_sx126x_bsp_get_xosc_cfg(const void* context, ral_xosc_cfg_t* xosc_cfg, sx126x_tcxo_ctrl_voltages_t* supply_voltage, uint32_t* startup_time_in_tick) {
	const struct device *dev = context;
	const struct sx126x_hal_config_t *config = dev->config;

	*xosc_cfg = config->xosc_cfg;
	*supply_voltage = config->tcxo_voltage;
	*startup_time_in_tick = sx126x_convert_timeout_in_ms_to_rtc_step(config->tcxo_wakeup_time_ms);
}
...
void ral_sx126x_bsp_get_trim_cap(const void* context, uint8_t* trimming_cap_xta, uint8_t* trimming_cap_xtb) {
	...
	if (config->capa_xta != 0xFF) *trimming_cap_xta = config->capa_xta;
	if (config->capa_xtb != 0xFF) *trimming_cap_xtb = config->capa_xtb;
}
...
void ral_sx126x_bsp_get_lora_cad_det_peak(const void *context, ral_lora_sf_t sf, ral_lora_bw_t bw, ral_lora_cad_symbs_t nb_symbol, uint8_t* in_out_cad_det_peak) {
...
__weak ral_status_t ral_sx126x_bsp_get_instantaneous_tx_power_consumption(const void *context, const ral_sx126x_bsp_tx_cfg_output_params_t *tx_cfg_output_params, sx126x_reg_mod_t radio_reg_mode, uint32_t *pwr_consumption_in_ua) {
  // SX1261
...
__weak ral_status_t ral_sx126x_bsp_get_instantaneous_gfsk_rx_power_consumption(const void *context, sx126x_reg_mod_t radio_reg_mode, bool rx_boosted, uint32_t *pwr_consumption_in_ua) {
	// TODO: find the br/bw dependent values

  if (radio_reg_mode == SX126X_REG_MODE_DCDC) {
    *pwr_consumption_in_ua = (rx_boosted)
                                 ? SX126X_GFSK_RX_BOOSTED_CONSUMPTION_DCDC
...
__weak ral_status_t ral_sx126x_bsp_get_instantaneous_lora_rx_power_consumption(const void *context, sx126x_reg_mod_t radio_reg_mode, bool rx_boosted, uint32_t *pwr_consumption_in_ua) {
  // TODO: find the bw dependent values

  if (radio_reg_mode == SX126X_REG_MODE_DCDC) {
    *pwr_consumption_in_ua = (rx_boosted)
                                 ? SX126X_LORA_RX_BOOSTED_CONSUMPTION_DCDC
